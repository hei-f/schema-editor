...
4. **异常处理**：如果某个步骤无法执行（如无改动需要提交），必须明确说明原因并标记为 `○ 步骤 X 跳过（原因：...）`
...
- **重要说明**：此检查必须在打包前完成，因为项目的 pre-push 钩子会执行 tsgo 检测，如果在打包后才发现类型错误，会导致需要重新打包
- 如果发现类型错误：
  - **立即终止发布流程**
...
- [x] 3.2 测试套件执行完成，结果：X passed / X failed / X skipped
...
- 如果tag已存在：
  - 检查tag指向：`git log --oneline --decorate | grep "tag: v{版本号}"`
  - 如果tag不在当前HEAD：
