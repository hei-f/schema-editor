import { FORM_PATHS } from '@/shared/constants/form-paths'
import { formatShortcut } from '@/shared/constants/keyboard-shortcuts'
import { DEFAULT_VALUES } from '@/shared/constants/defaults'
import type { ShortcutKey } from '@/shared/types'
import { KeyOutlined } from '@ant-design/icons'
import { Alert, Form, Space, Tooltip } from 'antd'
import React from 'react'
import { SectionCard } from '../../components/SectionCard'
import { ShortcutInput } from '../../components/ShortcutInput'
import { FormSectionLabelWithVariant } from '../../components/FormSectionLabelWithVariant'
import { FormContent, FormSection, HelpTooltipIcon } from '../../styles/layout.styles'
import type { SectionProps } from '../../types'

/** 生成带样式的 tooltip 说明 */
const renderTooltip = (description: string, defaultShortcut: ShortcutKey) => (
  <>
    {description}（默认：{formatShortcut(defaultShortcut)}）
  </>
)

/**
 * 快捷键配置区块
 * 配置 Schema 编辑器抽屉内的快捷键
 */
export const KeyboardShortcutsSection: React.FC<SectionProps> = (props) => {
  const { sectionId, isActive, onActiveChange, onResetDefault } = props

  return (
    <SectionCard
      title="快捷键配置"
      subtitle="配置 Schema 编辑器内的快捷键"
      icon={KeyOutlined}
      panelKey="keyboard-shortcuts"
      sectionId={sectionId}
      isActive={isActive}
      onActiveChange={onActiveChange}
      onResetDefault={onResetDefault}
    >
      <FormSection>
        <FormSectionLabelWithVariant id="field-editor-shortcuts">
          编辑器快捷键
        </FormSectionLabelWithVariant>
        <FormContent>
          <Form.Item
            label={
              <Space>
                保存
                <Tooltip
                  title={renderTooltip('保存当前编辑内容', DEFAULT_VALUES.drawerShortcuts.save)}
                >
                  <HelpTooltipIcon />
                </Tooltip>
              </Space>
            }
            name={FORM_PATHS.drawerShortcuts.save}
          >
            <ShortcutInput
              placeholder="录入保存快捷键"
              defaultValue={DEFAULT_VALUES.drawerShortcuts.save}
            />
          </Form.Item>
          <Form.Item
            label={
              <Space>
                格式化
                <Tooltip
                  title={renderTooltip('格式化 JSON 内容', DEFAULT_VALUES.drawerShortcuts.format)}
                >
                  <HelpTooltipIcon />
                </Tooltip>
              </Space>
            }
            name={FORM_PATHS.drawerShortcuts.format}
          >
            <ShortcutInput
              placeholder="录入格式化快捷键"
              defaultValue={DEFAULT_VALUES.drawerShortcuts.format}
            />
          </Form.Item>
          <Form.Item
            label={
              <Space>
                打开/更新预览
                <Tooltip
                  title={renderTooltip(
                    '打开预览或更新预览内容',
                    DEFAULT_VALUES.drawerShortcuts.openOrUpdatePreview
                  )}
                >
                  <HelpTooltipIcon />
                </Tooltip>
              </Space>
            }
            name={FORM_PATHS.drawerShortcuts.openOrUpdatePreview}
          >
            <ShortcutInput
              placeholder="录入打开/更新预览快捷键"
              defaultValue={DEFAULT_VALUES.drawerShortcuts.openOrUpdatePreview}
            />
          </Form.Item>
          <Form.Item
            label={
              <Space>
                关闭预览
                <Tooltip
                  title={renderTooltip('关闭预览面板', DEFAULT_VALUES.drawerShortcuts.closePreview)}
                >
                  <HelpTooltipIcon />
                </Tooltip>
              </Space>
            }
            name={FORM_PATHS.drawerShortcuts.closePreview}
          >
            <ShortcutInput
              placeholder="录入关闭预览快捷键"
              defaultValue={DEFAULT_VALUES.drawerShortcuts.closePreview}
            />
          </Form.Item>
          <Alert
            message="快捷键说明"
            description={
              <div>
                <p>1. 点击输入框后直接按下想要的快捷键组合</p>
                <p>2. 快捷键必须包含 Ctrl/Cmd 或 Alt 修饰键</p>
                <p>3. 避免使用浏览器保留快捷键（如 Cmd+W、Cmd+T）</p>
                <p>4. 快捷键仅在编辑器获得焦点时生效</p>
              </div>
            }
            type="info"
            showIcon
          />
        </FormContent>
      </FormSection>
    </SectionCard>
  )
}
